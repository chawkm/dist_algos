defmodule App6 do
    def start(n, peers_list, id_peer_map, erb) do
        state = %{:id => n, :peers => peers_list, :rb => erb, :id_peer_map => id_peer_map}
    ...
    after
        0 ->
            if state[:max_broadcasts] <= 0 do
                ...
                send state[:rb], {:rb_broadcast, :message}
...
def finish(%{:id => id, :sent => sent, :received => received} = state) do
    ...
    Process.exit(self(), "App terminated.")
end
